...
import { backupState, loadState, saveState } from './persistence';
...
export class Engine<S> {
  ...
  get emergencyActive(): boolean {
    return this.world.emergency?.(this.state) === true;
  }
  ...
  constructor(world: WorldModule<S>, opts: EngineOptions) {
    ...
    this.stopWorld = this.world.start?.(this.ctx) || undefined;
    this.log.push(`world '${world.meta.id}' up (${world.meta.name})`);
  }

  stop(): void {
    this.stopWorld?.();
    this.stopWorld = undefined;
  }

  /**
   * Operator reset: back the current world up, then start it over fresh.
   * Refuses to reset if the backup cannot be written — a reset must never be
   * the way a world gets lost.
   */
  reset(): { backup: string | null } {
    let backup: string | null;
    try {
      this.save();
      backup = backupState(this.world, this.dataDir, 'before-reset');
    } catch (e) {
      throw new Error(`Not resetting: could not back up the current world (${(e as Error).message})`);
    }
    this.stop();
    const now = Date.now();
    this.state = this.world.reset ? this.world.reset(this.state, now) : this.world.createInitialState(now);
    this.tasks.clear();
    this.protagonist.walk = undefined;
    this.protagonist.x = this.protagonist.homeX;
    this.protagonist.state = 'idle';
    this.speech = null;
    this.recentChat = [];
    this.recentSelfLines = [];
    this.classifications = [];
    this.stopWorld = this.world.start?.(this.ctx) || undefined;
    this.save();
    this.log.push(
      `world reset by operator (backup: ${backup ?? 'none — there was no saved world yet'})`,
      'alert',
    );
    this.markDirty();
    return { backup };
  ...
  async generateDialogue(req: DialogueRequest): Promise<string | null> {
    ...
    try {
      ...
      if (!line) {
        this.log.push(
          `dialogue produced no speakable line; raw: ${JSON.stringify(out.slice(0, 120))}`,
          'warn',
        );
        return null;
      }
      ...
      if (line !== out.trim()) {
        this.log.push(
          `stripped non-spoken text from dialogue; raw: ${JSON.stringify(out.slice(0, 160))}`,
          'warn',
        );
      }
      return line;
  ...
  buildView(now: number): EngineView {
    ...
    return {
      ...
      recentChat: this.recentChat.map(({ username, text, receipt }) => ({
        username,
        text,
