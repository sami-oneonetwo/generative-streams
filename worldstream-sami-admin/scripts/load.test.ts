...
test('a versioned snapshot that fails the schema refuses to boot instead of resetting the room', () => {
  ...
  // A genuinely unreadable file is different: nothing to preserve, so boot.
  writeFileSync(statePath(dir, world.meta.id), 'not json at all');
  const fresh = loadState(world, dir, (line) => logged.push(line));
  assert.equal(Object.keys(fresh.servers).length, 0);
